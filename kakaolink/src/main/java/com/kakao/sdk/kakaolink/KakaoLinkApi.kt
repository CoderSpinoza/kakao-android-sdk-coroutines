...
interface KakaoLinkApi {
    ...
    fun validateCustom(@Query(Constants.TEMPLATE_ID) templateId: String,
                       @Query(Constants.TEMPLATE_ARGS) templateArgs: Map<String, String>? = null): Deferred<KakaoLinkResponse>

    @GET("${Constants.VALIDATE_DEFAULT_PATH}?link_ver=4.0")
    fun validateDefault(@Query(Constants.TEMPLATE_OBJECT) templateObject: DefaultTemplate): Deferred<KakaoLinkResponse>
